...
def create_voice_call_response(
    ...
    twilio_asr_lang = resolve_twilio_asr_language(bot)
    if twilio_asr_lang:
        gather = Gather(
...
def resolve_twilio_asr_language(bot: TwilioVoice) -> str | None:
    # custom asr model specified: use it
    if bot.saved_run.state.get("asr_model"):
        return None
    # asr model not supplied, try to use twilio inbuilt asr
    user_language = (
        bot.saved_run.state.get("user_language") or TWILIO_DEFAULT_ASR_LANGUAGE
    )
    try:
        return normalised_lang_in_collection(
            user_language, TWILIO_SUPPORTED_ASR_LANGUAGES
        )
    except UserError:
        return None
...
def resp_say_or_tts_play(
    ...
    *,
    ...
    twilio_tts_voice = resolve_twilio_tts_voice(bot)
    ...
    resp.say(text, voice=twilio_tts_voice or TWILIO_DEFAULT_VOICE)


def resolve_twilio_tts_voice(bot: TwilioVoice) -> str | None:
    # bot is not using any TTS provider, use twilio inbuilt tts with default voice
    tts_provider = bot.saved_run.state.get("tts_provider")
    if not tts_provider:
        return TWILIO_DEFAULT_VOICE

    # bot is not using Google TTS, use gooey tts
    google_voice_name = bot.saved_run.state.get("google_voice_name")
    if not google_voice_name or tts_provider != TextToSpeechProviders.GOOGLE_TTS.name:
        return None

    # voice is not supported by twilio, use gooey tts
    twilio_voice = "Google." + google_voice_name
    if twilio_voice not in TWILIO_SUPPORTED_VOICES:
        logger.warning(f"Unsupported {twilio_voice=} for {bot.bi=}, using gooey tts")
        return None
