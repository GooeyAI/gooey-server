...
def generate_text_fragment(text, min_len=0, max_len=30):
    ...
    def find_alphanumeric_window(s, window_size, reverse=False):
        range_func = range(len(s) - window_size + 1, 0, -1) if reverse else range(len(s) - window_size + 1)
        for i in range_func:
            ...
            if re.match(r'^[a-zA-Z0-9\s"\'.,?:;()&^*$@!#]+$', window) and not re.search(r'[\[\].]', window):
                words = window.strip().split()
                if words and re.match(r'^[a-zA-Z0-9]+$', words[0]) and re.match(r'^[a-zA-Z0-9]+$', words[-1]):
                    return window.strip()
