...
def gooey_builder_send_message(request: fastapi.Request, body: GooeyBuilderSendMessage):
    ...
    builder_run_url = body.builder_run_url or get_default_builer_run_url()
...
def get_default_builer_run_url() -> str:
    try:
        bi = BotIntegration.objects.get(id=settings.GOOEY_BUILDER_INTEGRATION_ID)
    except BotIntegration.DoesNotExist:
    ...
    return bi.published_run.get_app_url()
...
def fetch_builder_conversations(
    ...
    qs = SavedRun.objects.order_by("-created_at").filter(
        parent_builder_saved_run__parent_version__published_run=pr,
        uid=request.user.uid,
    )[: body.limit]

    return [
        dict(
            title=sr.parent_builder_saved_run.state.get("input_prompt") or "",
